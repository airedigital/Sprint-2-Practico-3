...
export async function actualizar(req, res) {
    try {
        const nombre = req.params.nombre;
        const result = await superHeroRepository.actualizarPorNombre(
            nombre,
            req.body,
        );
        ...
        res.status(200).json(
            renderizarResultadoOperacion('Superhéroe actualizado', result),
        );
    } catch (error) {
        res.status(500).json({
...
export async function eliminar(req, res) {
    try {
        const nombre = req.params.nombre;
        const result = await superHeroRepository.eliminarPorNombre(nombre);
        ...
        res.status(200).json(
            renderizarResultadoOperacion('Superhéroe eliminado', result),
        );
    } catch (error) {
        res.status(500).json({
